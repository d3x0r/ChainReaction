...
function initFoyerForm() {
...

	{
			l.winstate = null;
			_debug && console.log( "got setup" );
			l.rows = Number(myGameHeight.textContent);
			l.cols = Number(myGameWidth.textContent);
			l.players.length = 0;
			foyerForm.hide();
			...
			gameStatusForm.players = Number(myGamePlayers.textContent);
			initBoard();
			gameStatusForm.update();
			gameStatusForm.show();
			gameBoardForm.show();
...
//-------------------- Server Connection ----------------------
function openSocket() {
	var ws = new WebSocket((location.protocol==="http:"?"ws://":"wss://")+location.host+"/", "ChainReaction");
